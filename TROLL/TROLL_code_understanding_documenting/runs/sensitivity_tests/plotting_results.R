# -------------------------------------------
# Script to plot results from TROLL simulations with different WTD values and
# distinct soil layer thickness
# Unified vertical water movement
...
#scenario_paths <- c("shallow_thin_sandy/outputs")
scenario_paths <- main_path
...
safe_read <- function(full_dir, file_type, label) {
  file_path <- file.path(full_dir, paste0(scenario_name,"0_", file_type, ".txt"))
...
plot_SWP_zoom_layers <- function(scenario_paths_vec = scenario_paths,
  ...
  message("Zoom Y-limits: ", round(y_range[1], 4), " to ", round(y_range[2], 5))
