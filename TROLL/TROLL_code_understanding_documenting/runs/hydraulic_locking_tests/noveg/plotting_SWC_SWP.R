  ...
  # Paths (ONE situation)
  # -----------------------------
  model_path <- "~/Desktop/Postdoc_Toulouse/Postdoc_Toulouse/TROLL/TROLL_code_understanding_documenting/runs/hydraulic_locking_tests/teste/output/"
  out_prefix <- "(null)"
  
  files_df <- tibble(
    model_name    = "run1",  # qualquer nome (só pra facetar)
    experiment_dir = model_path,
    pedology_path  = "~/Desktop/Postdoc_Toulouse/Postdoc_Toulouse/TROLL/TROLL_code_understanding_documenting/runs/hydraulic_locking_tests/noveg/common_inputs/Paracou_input_pedology.txt",
